...
class Outcar(object):
    def __init__(self, filename, parse_mag=False):
        ...
        exet_press_patt = re.compile("external.*?=\s+([-\d\.]+)")
        ion_toten_patt = re.compile("% ion.*\s+([-\d\.]+)")
        ...
        for line in chunks:
            ...
            if m:
                ionic_step, electronic_step = int(m.group(1)), int(m.group(2))
                continue
            ...
            m = exet_press_patt.search(line)
            if m:
                exet_press.append(float(m.group(1)))
            m = ion_toten_patt.search(line)
            if m:
                ion_toten.append(float(m.group(1)))
        ...
        self.electronic_step = electronic_step
        ...
        self.exet_press = np.array(exet_press)
        self.ion_toten = np.array(ion_toten)
