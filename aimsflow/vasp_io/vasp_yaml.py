...
class VaspYaml(OrderedDict):
    ...
    @staticmethod
    def generate_from_vasp_files(work_dir, incar=None, poscar=None, name_suffix=None,
                                 folder_name=None, walltime=None):
        vasp_input = OrderedDict()
        poscar = Poscar.from_file('%s/POSCAR' % work_dir) if poscar is None else poscar
        batch_run = BatchFile.from_file('%s/runscript.sh' % work_dir)
        if walltime:
            batch_run.update({TIME_TAG: walltime})
        if name_suffix:
            if MANAGER == 'PBS':
        ...
        if incar.get('NSW', 0) > 1:
            ...
            if incar.get('ISMEAR', 1) != 0:
                ...
                kpt = Kpoints.from_file('%s/KPOINTS' % work_dir)
    ...
    def create_vasp_files(self, jp, jt, jn, poscar, next_jt='', command='',
        ...
        if self.get('ADD_MAG') and incar.get('MAGMOM') is None:
            mag = []
            magmom = copy.copy(VASP_CONFIG['MAGMOM'])
            if self.get('MAGMOM'):
                magmom.update(**self['MAGMOM'])
        ...
        if self.get('ADD_U'):
            ...
            for k in ('LDAUU', 'LDAUJ', 'LDAUL'):
                temp = copy.copy(VASP_CONFIG[k])
                if self.get(k):
                    temp.update(**self.get(k))
                incar[k] = [temp.get(sym, 0) for sym in poscar.site_symbols]
