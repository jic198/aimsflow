...
def queue(args):
    ...
    if args.launch:
        ...
        for folder in folders:
            ...
            bf.update({'command': f'cd $SLURM_SUBMIT_DIR\nmv * {ipath}\n'})
...
def main():
    ...
    group.add_argument('-c', '--clean', action='store_true',
                       help='Clean VASP files to only submission files.\n'
                            'E.g. aimsflow vasp -c')
    group.add_argument('-k', '--kill', nargs='+',
                       help="Kill submitted VASP job in 'ID_list' or specified jobs.\n"
                            "E.g. aimsflow vasp -k ID_list")
    ...
    parser_vasp_job.add_argument('-mr', '--max_run', type=int, default=5,
                                 help='Maximum number of rerun\n'
                                      'E.g. aimsflow vasp -cj rs -mr 10')
    ...
    parser_analyze.add_argument('-cv', '--converge', nargs='?', choices=['no', 'yes'],
                                const='no', help='Check converged and un-converged '
                                                 'calculation \nE.g. aimsflow analyze -cv yes')
    ...
    pdos_parser.add_argument('-st', '--split_type', default='spdf', const='spdf', nargs='?',
    ...
    parser_job_yaml.add_argument('-mr', '--max_run', type=int, default=5,
                                 help='Maximum number of rerun\n'
                                      'E.g. aimsflow jyaml -g rs -mr 10')
    ...
    slab_parser = build_subparser.add_parser(
        'slab', help='Slab\nE.g. aimsflow build slab POSCAR 1,1,1 -v 5 -u 2 -dl 1b1t',
        formatter_class=argparse.RawTextHelpFormatter)
    ...
    slab_parser.add_argument('-s', '--shift', type=float, default=0.0, const=0.0, nargs='?',
                             help='A shift value in fractional coordinates that determines '
                                  ...
                                  'slab POSCAR 1,1,1 -v 15 -s 0.1')
    slab_parser.add_argument('-center', '--center', action='store_true',
                             help='center the slab in the cell with equal vacuum spacing '
                                  'from the top and bottom\nE.g. aimsflow build '
                                  'slab POSCAR 1,1,1 -v 15 -center')
    slab_parser.add_argument('-conventional', '--conventional', action='store_true',
                             help='center the slab in the cell with equal vacuum spacing '
                                  'from the top and bottom\nE.g. aimsflow build '
    ...
    remove_parser = build_subparser.add_parser(
        'remove', help='Remove species, sites or layers\nE.g. aimsflow build '
