...
class Slab(Structure):
    def __init__(self, lattice, species, coords, miller_index,
        ...
        lattice = Lattice.from_lengths_angles(*lattice.lengths_and_angles)
...
class SlabGenerator(object):
    ...
    def get_slab(self, delete_layer="0b0t", shift=0.0, tol=0.25):
        nlayers = self.slab_uc + int(np.ceil(self.vacuum / self._proj_height))
        ...
        frac_coords = np.array(frac_coords) + np.array([0, 0, -shift])
        ...
        if self.center_slab:
            ...
            slab.translate_sites([0, 0, 0.5 - avg_c], (list(range(len(slab)))))
        if self.primitive and self.vacuum > 0:
