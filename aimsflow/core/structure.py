...
class IStructure(SiteCollection):
    ...
    def sort_sites_in_layers(self, tol=0.25, reverse=False, axis=2):
        ...
        if self.lattice.abc[axis] * abs(
            ...
            new_layers = [sorted(tmp)] + new_layers[1:-1]
...
class Structure(IStructure, collections.MutableSequence):
    ...
    def modify_only_lattice(self, new_lattice):
        self._lattice = new_lattice
        new_sites = []
        for i, s in enumerate(self._sites):
            new_sites.append(PeriodicSite(
                s.specie, s.frac_coords, self._lattice, properties=s.properties))
        self._sites = new_sites

    def scale_lattice(self, volume):
        self.modify_only_lattice(self._lattice.scale(volume))
    ...
    def get_primitive_structure(self, tolerance=0.25, use_site_props=False,
        ...
        grouped_sites = [
            list(a[1])
            for a in groupby(sites, key=site_label)]
                ...
                for a in factors(det):
                    for e in factors(det // a):
                        ...
                        yield det, np.array(
                            [[[a, b, c], [0, e, f], [0, 0, g]]
                             for b, c, f in
                             product(range(a), range(a), range(e))])
